...
def draw():
    ...
    for y in range(GRID_ROWS):
        for x in range(GRID_COLS):
            ...
            if val == 1:
                ...
                base_color = COST2_C      # CONS TETAP KELIHATAN
            elif val == 3:
            ...
            # 2️⃣ explored = overlay transparan
            if (x, y) in explored:
                screen.blit(explored_surf, rect.topleft)

    # ===== DRAW PATH (DI ATAS EXPLORED) =====
    for (px, py) in path:
        screen.blit(path_surf, grid_to_screen(px, py))
    ...
    pygame.draw.rect(
        screen, START_C,
        pygame.Rect(*grid_to_screen(sx, sy), CELL_SIZE, CELL_SIZE)
    )
    pygame.draw.rect(
        screen, GOAL_C,
        pygame.Rect(*grid_to_screen(gx, gy), CELL_SIZE, CELL_SIZE)
    )
    ...
    pygame.draw.rect(
        screen, NPC_C,
        pygame.Rect(*grid_to_screen(nx, ny), CELL_SIZE, CELL_SIZE),
        border_radius=6
    )
